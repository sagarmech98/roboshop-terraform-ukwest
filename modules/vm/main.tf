...
resource "null_resource" "anisble" {
  ...
  provisioner "remote-exec" {
    inline = [
        ...
        "ansible-pull -i localhost, -U https://github.com/sagarmech98/roboshop-ansible roboshop.yaml -e role_name=${local.role_name} -e app_name=${var.name} -e env=dev -e token=${var.token}"  
    ]
    }
  }
